...
Si è imposta una smoothing length adattiva variabile per ogni particella in funzione della densità locale; h viene calcolata dinamicamente affinchè il nucleo di interpolazione contenza un numero di vicini pari a 64 particelle. Il calcolo dei vicini viene implelentato secondo due modalità:
1. Ricerca QuickSort (QSORT): tutte le particelle vengono riordinate lungo x;
...
L'obiettivo è di paragonare l'efficienza temporale di esecuzione delle due modalità. 
...
1. Metodo Quicksort \
    ...
    II. Plot pressione iniziale e finale (shock_tube_rho.png); \
